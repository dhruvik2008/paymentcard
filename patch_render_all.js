...
const jsCode = `
  ...
  const renderAllTransactions = () => {
    ...
    // Populate Filters Options dynamically
    ...
    transactions.forEach(tx => {
      ...
      const fullDate = \`\${tx.date} \${tx.time}\`;
      ...
      if (tx.raw && tx.raw.debits) {
        tx.raw.debits.forEach(d => {
          ...
          allTx.push({
            type: 'Credit',
            dateStr: d.date,
            timeStr: tx.time, // Using transaction time as fallback
            timestamp: new Date(\`\${d.date} \${tx.time}\`).getTime(),
      ...
      if (tx.raw && tx.raw.payments) {
        tx.raw.payments.forEach(p => {
          ...
          allTx.push({
            ...
            dateStr: p.date,
            timeStr: tx.time,
            timestamp: new Date(\`\${p.date} \${tx.time}\`).getTime(),
            customer: tx.customerName,
            bank: bankInfo,
            portal: p.portal || 'N/A',
            baseAmount: amt,
    ...
    const selEnd = ledgerEndDate.value ? new Date(ledgerEndDate.value).getTime() + 86400000 : Infinity; // Include end day
    ...
    // Calculate Running Balance on filtered data
    ...
    filteredTx.forEach(tx => {
      ...
      if (isCredit) {
        ...
        chargesHtml = \`<div style="color: #6b7280; font-size: 0.85rem;">No charges</div>\`;
      }
      ...
      tr.innerHTML = \`
        ...
        <td style="vertical-align: top; text-align: right; color: #10b981; font-weight: 700; font-size: 1.05rem;">
...
console.log("Injected renderAllTransactions");
