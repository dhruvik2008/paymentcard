...
const ITEMS_PER_PAGE = 20;
  ...
  window.updateWizardDebit = (idx, field, val) => {
    ...
    // Auto-calculate Charges based on Portal % & Amount
    if (field === 'amount' || field === 'portalPercent') {
      const amt = parseFloat(wizardDebits[idx].amount) || 0;
      const portalRate = parseFloat(wizardDebits[idx].portalPercent) || 0;
      if (amt > 0 && portalRate > 0) {
        const charges = (amt * portalRate / 100).toFixed(2);
    ...
    // Auto-calculate Portal % based on Charges & Amount
    else if (field === 'charges') {
      const amt = parseFloat(wizardDebits[idx].amount) || 0;
      const charges = parseFloat(wizardDebits[idx].charges) || 0;
      if (amt > 0 && charges >= 0) {
        const portalRate = ((charges / amt) * 100).toFixed(2);
        wizardDebits[idx].portalPercent = portalRate;
        const portalPercentInput = document.getElementById('debit-portalPercent-' + idx);
        if (portalPercentInput) portalPercentInput.value = portalRate;
      }
    }
  ...
  window.selectDebitPortal = (idx, portal) => {
    ...
    if (typeof currentPortals !== 'undefined') {
      ...
      if (pInfo) {
        ...
        const amt = parseFloat(wizardDebits[idx].amount) || 0;
        const pRate = parseFloat(pInfo.portalPercent) || 0;
        if (amt > 0 && pRate > 0) {
          wizardDebits[idx].charges = (amt * pRate / 100).toFixed(2);
        }
      }
    }
